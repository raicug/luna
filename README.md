...
- Manufactured instances inside a returned table or pack via `OwnedValue::Instance<T>` and `ReturnPack::AppendInstance<T>`, so a `GetChildren()`-shaped result and an `Iterate` that yields objects both work
