// Property 22 of the reflection-driven binding system: every canonical
// conversion round-trips through its own type-specific equivalence model, and
// any injected nested failure reports the complete path while exposing neither
// a partial native aggregate nor a partial virtual-machine result.
//
// Generated dimensions: the foundation scalars, the planned `float`, string
// view, and C string forms, optionals, sequences, fixed arrays, associative
// maps, pairs, tuples, registered enumerations, registered class handles,
// nested aggregates, argument packs, and zero, one, and many return packs.
...
// Deterministic byte source. Every generated shape is derived from one
// RapidCheck byte vector, so shrinking keeps producing a smaller shape rather
...
// One canonical type built from the generated bytes. Nesting stops at `Depth`,
// which is an observation bound of the test rather than a conversion policy.
...
// One generated value in both vocabularies: the Luau value a script would pass
// and the staged native value a converter should produce from it.
...
[[nodiscard]] StagedValue GenerateLeafValue(const TypeDescriptor &Type,
  ...
  case FixedTypeKey::Float: {
    // A single-precision leaf narrows exactly once, so the model narrows too.
...
StagedValue GenerateValue(const TypeDescriptor &Type, SeedCursor &Seeds) {
  ...
  case TypeKind::Optional:
    // Absence is covered by its own fixed sub-case; a generated optional is
    // present so a nested element never vanishes from its table.
  ...
  case TypeKind::Map: {
    // Keys are generated distinct and already in canonical order, which is the
    // order the reader stages entries in.
...
// One Luau value the canonical type refuses, used to inject a nested failure at
// exactly the position the model predicts.
...
// Values one return shape publishes: zero for void, one per ordered element of
// a root pair, tuple, or return pack, and one otherwise.
...
int RunCanonicalConversionRoundTripProperties() {
  ...
  const bool Passed = rc::check(
      ...
      [](const std::vector<std::uint8_t> &GeneratedSeeds,
        ...
        // -- the registry describes the generated type deterministically ----
        ...
        // The foundation five keep their exact names and exact rank.
        ...
        // -- reading is equivalent to the model and leaves no value behind --
        ...
        // -- writing publishes exactly one value and round-trips -----------
        ...
        // -- zero, one, and many return shapes -----------------------------
        ...
        // -- nil and absence, which only a root position can carry ---------
        const auto NullRead =
        ...
        // -- an injected nested read failure reports the complete path ------
        ...
        // -- an injected nested write failure publishes nothing -------------
        ...
        // -- a class handle without its registered class refuses ------------
        ...
        // -- probing has no side effect on the boundary --------------------
        ...
        Luna::Detail::ResetConversionBoundaryDiagnostics();
        {
          ...
          // A well-behaved probe reads shape only and records nothing.
          ...
          // A probe that reaches a mutating operation is refused and recorded,
          // and the frame keeps no partial result.
          ...
          // The committing context still reserves and publishes exactly once.
        ...
        // -- a State stays usable after all of that conversion work ---------
        Luna::State State;
