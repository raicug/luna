...
The public entry point is `<luna/luna.hpp>`. A `Luna::State` owns one Luau VM, `State::Bindings()` returns the `BindingRegistry` every declaration goes through, `State::Execute()` compiles and runs source, and `State::Load()` compiles source into a reusable `Luna::Chunk`. Consumer code never needs a Luau header, VM pointer, stack operation, or compile definition.
...
Chunks and re-entrancy are available. `State::Load` compiles source into a `Luna::Chunk` owning validated bytecode; `Chunk::Invoke` takes a `ValuePack` and reports the values the chunk returned. A chunk may be invoked from inside a native call, on its own Luau thread but sharing the State's globals and dispatch generation, bounded at 16 nesting levels; and a callable returning a `Chunk` publishes it to a script as a real Luau function, which is how `loadstring` is expressed. Execution is interruptible from any thread through `State::RequestInterrupt`. See [Executing Luau](executing-luau.md#loading-a-chunk).
